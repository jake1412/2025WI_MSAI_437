...
def load_mnist(train_batch_size: int = 64, test_batch_size: int = 1000):
    ...
    cuda_kwargs = {"num_workers": 1, "pin_memory": True, "shuffle": True} if torch.cuda.is_available() else {}
    ...
    # load MNIST data
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=train_batch_size, **cuda_kwargs)
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=test_batch_size, **cuda_kwargs)
...
def load_audiomnist(data_dir, train_batch_size: int = 64, test_batch_size: int = 128):
    ...
    if len(cache_list) > 0:
        ...
        for i, audio_fn in enumerate(pbar):
            pbar.set_description(f"Loading AudioMNIST ({os.path.basename(audio_fn)})")
            waveform, _ = li.load(audio_fn, mono=True, sr=16000, duration=1.0)
